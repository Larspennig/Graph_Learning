...
class TransformerGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        x_1.edge_index = graph.create_graph(data=x_1, factor=5)

        x_2 = self.enc2(x_1)
        graph = Skewed_Gauss_Graph(model=self, enc='enc3')
        x_2.edge_index = graph.create_graph(data=x_2, factor=5)

        x_3 = self.enc3(x_2)
        graph = Skewed_Gauss_Graph(model=self, enc='enc4')
        x_3.edge_index = graph.create_graph(data=x_3, factor=5)

        x_4 = self.enc4(x_3)
        graph = Skewed_Gauss_Graph(model=self, enc='enc5')
        x_4.edge_index = graph.create_graph(data=x_4, factor=5)
