...
class TransformerGNN(nn.Module):
    def __init__(self, config):
        ...
        # Build encoder stack
        self.enc1 = self._make_encoder(blocks = blocks[0], 
                                       channels=channels[0], 
                                       value_down=value_down[0], 
                                       subsampling=subsampling)
        self.enc2 = self._make_encoder(blocks = blocks[1], 
                                       channels=channels[1], 
                                       value_down=value_down[1], 
                                       subsampling=subsampling)
        self.enc3 = self._make_encoder(blocks = blocks[2], 
                                       channels=channels[2], 
                                       value_down=value_down[2], 
                                       subsampling=subsampling)
        self.enc4 = self._make_encoder(blocks = blocks[3], 
                                       channels=channels[3], 
                                       value_down=value_down[3], 
                                       subsampling=subsampling)
        self.enc5 = self._make_encoder(blocks = blocks[4], 
                                       channels=channels[4], 
                                       value_down=value_down[4], 
                                       subsampling=subsampling)


        # Build decoder stack
        self.dec1 = self._make_decoder(blocks = 1, 
                                       channels = channels[4], 
                                       special = 'one_input')
        self.dec2 = self._make_decoder(blocks = 1, 
                                       channels = channels[3])
        self.dec3 = self._make_decoder(blocks = 1, 
                                       channels = channels[2])
        self.dec4 = self._make_decoder(blocks = 1, 
                                       channels = channels[1])
        self.dec5 = self._make_decoder(blocks = 1, 
                                       channels = channels[0])
